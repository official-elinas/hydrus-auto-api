...
for file in os.listdir(original_tags_dir):
    with open(f"{original_tags_dir}/{file}", 'rb') as f:
        ...
        for line in lines:
            ...
            if 'creator:' in tag and not creator_found:
            ...
            if ('character:' in tag) and (tag != 'character:original') and tag.split(':')[1] not in found_tags: 
            ...
            if ':' not in tag and not tag.isnumeric() and tag.isascii():  # and not tag.isnumeric()
                found_tags.append(tag)
